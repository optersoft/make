...
default that a consumer overrides. Duplicate tasks across imports are fatal
there, which is why `play-test-gate` had to be left *undefined* upstream.
"""

from __future__ import annotations

from make import env, invoke, task
...
def test_a_shared_default_is_replaced_by_the_consumers_override():
    ...
    @task(group="web", name="preflight")
    def preflight() -> None:
        calls.append("shared default")

    # ...in the consumer's Makefile.py
    @task(override="web.preflight")
...
def test_calling_the_imported_function_still_runs_that_function():
    """Tasks stay plain functions -- only `invoke` consults the registry."""
    calls: list[str] = []
...
def test_an_override_takes_over_the_targets_identity():
    """A consumer writes `def test_gate()`, but the task is still `play.test-gate`.
    ...
    @task(group="play", name="test-gate", abstract=True, aliases=["gate"])
    def test_gate() -> None: ...

    @task(override="play.test-gate")
    def my_gate() -> str:
        return "checked"
