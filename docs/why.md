...
is the evidence: one fleet's shared `just` library — seven `.just` files, 1112
lines, consumed by five repositories — and what its git history records.
...
- **Startup.** `just` starts in about 5 ms; `mkrun` takes about 30 ms. Both are
